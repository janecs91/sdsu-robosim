classdef WalkKinematics
    ...
    methods
        ...
        function [A, B, C, allowed] = get_ABC_swing(obj, activeLeg, altLeg, x_a_prime, y_a_prime, z_a_prime)
        ...
        function [dot_waist, dot_hip, dot_knee] = eval_alt_kinematics_swing2(obj, inputState)
            % sigma = slope of path
            % slope
            % not implemented?
            ...
            if A^2+B^2-C^2 < 0
                fprintf("dx_i: %.2f, dy_i: %.2f, dz_i: %.2f\n", dx_i, dy_i, dz_i);
                error("Delta part becomes imaginary number");
            end
        ...
        function [dot_waist, dot_hip, dot_knee] = eval_alt_kinematics_stance2(obj, state)
            % sigma = slope of path
            % slope
            activeLeg = state.activeLeg;
            altLeg = mod(activeLeg,2)+1;
            ...
            if C~=0
                delta_part2_phrase = (2*A*obj.a_4)^2+(2*B*obj.a_4)^2-C^2;
