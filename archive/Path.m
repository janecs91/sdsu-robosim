    ...
    methods
        function obj = Path(fileAddress, directory, pathArgs)
            ...
            try
                ...
                disp('error: could not find path. generating...');
            ...
            obj.pathName = PathGenerator.get_path_name(pathArgs{:});
        ...
        function [angle, slope] = get_gamma_at_point(obj, pathPoint)
            % not done
            slope = 0;
        ...
        function [futurePoint, futurePathIndex] = get_next_nearest_point(obj, x1, y1, lastIndex, accuracyRadius)
            %disp("GNNP====")
            %fprintf("input - x1: %.2f, y1: %.2f, lastind: %d, rad: %.2f\n", x1, y1, lastIndex, accuracyRadius);
            ...
            fprintf("path pt: %.2f %.2f, pathidx: %d, x1 y1: %.2f %.2f, x2 y2: %.2f %.2f\n", futurePoint, futurePathIndex, x1, y1, x2, y2);
        ...
        function y = get_path_y_at_x(obj, x)
            [nearestPathPointIndex, nearestPoint] = obj.get_nearest_point_index(x2, y2);
        end
