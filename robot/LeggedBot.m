    ...
    methods
        ...
        function obj = move_next_step(obj, stepVector, turnAngle, terrain, path)
            ...
            if obj.useAltStepAdjuster == true
                ...
                [dx_b, dy_b, dz_b, newBodyTurnAngle, futurePathIndex] = obj.stepAdjusterStance.get_step_vector2(obj, previousState, activeLeg, terrain, path);
            ...
            globalBodyVector(1:2) = obj.rotate_vector(relativeBodyVector(1:2), turnAngleBody+previousState.baseOrientation(3));
        ...
        function [obj, newState] = move_leg(obj, previousState, activeLeg, legVector, saveState)
            ...
            if legVector(1) == 0 && legVector(2) == 0 && legVector(3) == 0
                return;
            end
            maxI = max(floor(sum(abs(legVector))/2)+1, 1);
            if obj.iterateMovement == false
                maxI = 1;
            end
            legVectorI = legVector./maxI;
            %fprintf("#### ITERATE LEG MOVEMENT: maxI: %d // legVectorI: [%d %d %d]\n", maxI, legVectorI)
            for sequence=1:maxI
        ...
        function [obj, newState] = move_body(obj, previousState, activeLeg, bodyVector, ...
            ...
            newState = copy(previousState);
            if bodyVector(1) == 0 && bodyVector(2) == 0 && bodyVector(3) == 0
                return;
            end
