classdef StepAdjusterStance < StepAdjuster
    ...
    methods
        ...
        function [mu_dx_b, mu_dy_b, mu_dz_b, turnBodyAngle, futurePathIndex] = get_step_vector2(obj, bot, state, stanceLeg, terrain, path)
            state = bot.update_end_positions(state);
            jointPositions = bot.get_local_joint_positions(state, stanceLeg);
            jointPositions = jointPositions{1};
            z_a = jointPositions(end,3);
            x_a = jointPositions(end,1);
            y_a = jointPositions(end,2);
            footDistanceFromBase = bot.get_distance_base_to_foot(state, stanceLeg);
        ...
        function dz_b = get_base_z_vector(obj, bot, state, terrain, futurePoint)
            ...
            if dzFromBaseBottomToTerrain < obj.minBottomZFromTerrain
                dz_b_min = obj.minBottomZFromTerrain-dzFromBaseBottomToTerrain;
            end
    ...
    methods(Static)
        ...
        function isStable = check_stability_stance(leg, turnAngle, x_a, y_a, dx_b, dy_b)
            ...
            stabilityCheck = [stabilityCheckLeg1 stabilityCheckLeg2];
