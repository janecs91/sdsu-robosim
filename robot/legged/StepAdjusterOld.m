...
classdef StepAdjusterOld
    ...
    methods
        ...
        function isStable = is_stable_alt(obj, bot, state, leg, x_a, y_a, z_a, x_a_prime, y_a_prime, z_a_prime)
            ...
            % stance
            altLegs = setdiff(1:bot.numLegs, leg);
            ...
            isStable = stepSize < bot.legLength^2;
        ...
        function [stepSize, localLegVector] = adjust_leg_step_binary_search(obj, bot, state, slope, turnAngleBody, leg, terrain, path)
            ...
            endPos = bot.get_global_end_positions(testState, leg);
            oldZ = endPos(3);
            %maxStepSize = bot.get_max_step_size(slope, x_a, x_h, z_a, z_h, gamma);
            maxStepSize = bot.legLength/2;
            minStepSize = maxStepSize/10;
            stepSizes = linspace(maxStepSize, minStepSize, 10);
            fprintf("-- max stepsize: %.2f\n", maxStepSize);
            stepAdjust = bot.legLength/2;
            stepSize = 0;
            bodyVector = [20 0 0];
            [futurePoint, futurePathIndex] = bot.get_next_path_point(state, path);
            while abs(stepAdjust) > 1
                ...
                %[x_a_prime, y_a_prime, z_a_prime] = bot.get_new_prime(x_a, y_a, z_a, gammaLeg, legDistance);
                ...
                z_a_prime = z_a;
                isStable = bot.is_stable_alt(testState, leg, x_a, y_a, z_a, x_a_prime, y_a_prime, z_a_prime);
                sign = -1;
                if isStable
                    disp("stable")
                    sign = 1;
        ...
        function [localLegVector, futureEndPosition] = predict_leg_vector(obj, bot, previousState, activeLeg, bodyVector, turnAngle)
            ...
            %{
            bodyVectorLength = phaseBodyVector(1)./bot.numLegs;
            relativeLegVector = [distance - bodyVectorLength 0 0];
            relativeLegVector = bot.rotateVector(relativeLegVector(1:2), turnAngleLeg);
            %}
        end
    end
end