classdef StepAdjusterSwing < StepAdjuster
    ...
    methods
        function obj = StepAdjusterSwing(safetyValue, strategy)
            ...
            obj.frontCornerDistance = sqrt(2)*15.5;
        ...
        function [mu_dx_i, mu_dy_i, mu_dz_i] = get_step_vector2(obj, bot, state, leg, terrain, path)
            ...
            dwf = bot.get_distance_waist_to_foot(state, leg);
            dx_i = (obj.maxLegLength-dwf)+obj.frontCornerDistance;
            ...
            fprintf("local waist x: %.2f, dwf: %.2f, maxdx_i: %.2f\n", waist_x, dwf, dx_i);
            disp("testRange")
            disp(testRange);
            %}
            for i=testRange
