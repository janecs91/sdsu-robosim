    ...
    methods
        function obj = BaseBot()
            ...
            if ~strcmp(obj.mode, 'roll')
                obj.savePlannedEndPositions = true;
            end
        end
        ...
        function obj = add_history(obj, newState)
            ...
            if obj.savePlannedEndPositions
                %obj.plannedEndPositionsMatrix = cat(1, obj.plannedEndPositionsMatrix, newState.to_vector_planned_end_positions_flatten());
                obj.plannedEndPositionsMatrix = cat(3, obj.plannedEndPositionsMatrix, newState.to_vector_planned_end_positions());
            end
        end
        ...
        function obj = traverse_terrain(obj, terrain, path, startX, maxI)
            ...
            endX = path.pathPoints(end,1)-1;   
            if ~strcmp(obj.mode, 'roll')
                endX = endX-24;     % this is so that there are enough path points for legs to follow (pull, walk)
            end
            x = startX;
            i = 0;
            ...
            while x < endX && x < maxCenterX && i < maxI
                ...
                %disp(lastState.pathIndex)
                %disp(size(path.pathPoints))
                if lastState.pathIndex == size(path.pathPoints,1)
                    disp("Breaking")
                    break
                end
                i = i + 1;
                fprintf("[from basebot] x %.2f endX %.2f maxCenterX %.2f x<endX? %d x<maxCenterX %d x<maxI %d \n", x, endX, maxCenterX, x<endX, x<maxCenterX, x<maxI);
            end
        end
