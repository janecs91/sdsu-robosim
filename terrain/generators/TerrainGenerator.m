classdef TerrainGenerator
    properties(Constant)
        ...
        extraArgNames = {'amplitude', 'frequency', ...
                'randFilterSize', 'elevationChangeRange', 'path'};
    end
    ...
    methods
        ...
        function fileAddress = gen_from_options(obj, genFromPath, path, terrainType, width, height, ...
            ...
            %disp("varargin")
            %disp(varargin)
            generateTerrainArgs = varargin;
            generateTerrainArgs{end+1} = path;
            generateTerrainArgs = generateTerrainArgs(indices);
            %disp("TERRAIN INPUT")
            %disp(generateTerrainArgs)
            fileAddress = terrain.generate(xRange, yRange, scale, generateTerrainArgs{:});
        end
        function name = get_name(obj, fromPath, path, type, varargin)
            ...
            mapValues{end+1} = path.pathName;
            %disp(mapValues)
            map = containers.Map(allArgNames, mapValues,'UniformValues',false);
            if fromPath == true
                [width, height] = TerrainGenerator.get_terrain_size_from_path(path);
            else
                width = map('width');
